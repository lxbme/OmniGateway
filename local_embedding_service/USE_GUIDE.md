...
```bash
cd /your_path/OmniGateway/local_embedding_service/build_onnx
...
如果你只想启动 `embedding-service`，不拉起整套网关链路，可以单独构建并运行这个容器。

#### 方式 A：使用 `docker compose`
```bash
cd /your_path/go_projects/OmniGateway
docker compose up -d --build embedding-service
docker compose logs -f embedding-service
```

#### 方式 B：直接使用 `docker run`
```bash
cd /your_path/go_projects/OmniGateway
docker build -t local-embedding-service -f local_embedding_service/Dockerfile .
docker run --rm --name local-embedding-service \
  -p 50051:50051 \
  -e SERVE_PORT=50051 \
  -e EMBEDDING_BACKEND=mock \
  local-embedding-service
```

**说明**：
- 这个镜像默认是 Mock 后端，适合本地联调和快速验证。
- 容器启动后，服务会监听 `0.0.0.0:50051`，宿主机可以直接访问 `localhost:50051`。
- 如果你要验证容器里的服务，可在宿主机执行：
  ```bash
  cd /your_path/go_projects/OmniGateway/local_embedding_service
  bash check_embedding_service.sh localhost 50051
  ```

### 3.3 ONNX Runtime 模式（真实模型推理）
...
**Rerank 后端配置**：
```bash
# Mock rerank（默认，无需额外配置）
# 或启用 ONNX rerank：
export EMBEDDING_BACKEND=onnx
export LOCAL_RERANK_MODEL_PATH=/path/to/cross-encoder-model.onnx  # 必需
export LOCAL_RERANK_MODEL=cross-encoder-ms-marco-MiniLM-L6-v2    # 模型名称（默认）
export LOCAL_RERANK_MAX_LENGTH=512                                 # 最大序列长度（默认 512）
export LOCAL_RERANK_VOCAB_PATH=/path/to/vocab.txt                 # 可选，默认取模型目录下
export LOCAL_RERANK_PROVIDER=onnx-runtime                          # 提供者名称（默认）
```
...
```bash
cd /your_path/go_projects/OmniGateway/local_embedding_service/build_onnx
...
#### 1. Mock Rerank 后端（默认回退）
- **用途**: 基本排序验证
- **特点**: 基于 token overlap 的启发式排序
- **限制**: 无真实语义相关性评分

#### 2. ONNX Rerank 后端（可选）
- **用途**: 生产环境、真实文档重排序
- **特点**: 跨编码器（cross-encoder）模型推理，精确的 query-document 相关性评分
- **配置环境变量**:
  - `LOCAL_RERANK_MODEL_PATH` — ONNX rerank 模型文件路径
  - `LOCAL_RERANK_MODEL` — 模型名称（默认 `cross-encoder-ms-marco-MiniLM-L6-v2`）
  - `LOCAL_RERANK_MAX_LENGTH` — 最大序列长度（默认 512）
  - `LOCAL_RERANK_VOCAB_PATH` — vocab.txt 路径（可选，默认取模型目录下的 vocab.txt）
  - `LOCAL_RERANK_PROVIDER` — 提供者名称（默认 `onnx-runtime`）
...
2. 如果为 "onnx" 且编译时启用了 ONNX：使用 OnnxEmbeddingBackend / OnnxRerankBackend
3. 否则：使用 MockEmbeddingBackend / MockRerankBackend
4. 失败时自动降级到 Mock
